...
// TODO:
// Fix pacman and ghost movement, it starts okay but then he became crazy - OK
...
//
GLfloat material[4];
GLfloat vec_roof[4] = {165,42,42,1.0};
GLfloat vec_floor[4] = {0.0,1.0,0.0,1.0};
GLfloat vec_walls[4] = {210,105,30,1.0};
// vector for handling flashlight direction
...
int main(int argc,char *argv[]) {
    ...
    // Initialize pacman
    player.set_cellWidth(cell_width);
    ...
    glutInitWindowSize(cell_width * maze.maze_width + 200, cell_width * maze.maze_heigth + 200);
    ...
    LoadTexture("pared.jpg",128);

    glBindTexture(GL_TEXTURE_2D,1);
    LoadTexture("grass_tex.jpg",128);

    glBindTexture(GL_TEXTURE_2D,2);
    LoadTexture("roof_scaled.jpg",128);

	glBindTexture(GL_TEXTURE_2D,3);
    LoadTexture("yellow_tex.jpg",128);

	glBindTexture(GL_TEXTURE_2D,4);
    LoadTexture("blue_tex.jpg",128);

	glBindTexture(GL_TEXTURE_2D,5);
    LoadTexture("red_tex.jpg",128);

	glBindTexture(GL_TEXTURE_2D,6);
    LoadTexture("green_tex.jpg",128);
 ...
 void PositionObserver(float alpha,float beta,int radi) {
  ...
  //gluLookAt(x,y,z,    player.x, player.y, cell_width,upx,upy,upz);
}
 ...
 void display() {
	...
	for (int i = 0 ; i < ghost.size() ; i++) {
		glEnable(GL_TEXTURE_2D);
		glBindTexture(GL_TEXTURE_2D,4+i);
		sphere.draw(ghost[i].x, ghost[i].y, cell_width/2, ghost[i].r, ghost[i].g, ghost[i].b);
		...
		for (int y = 0; y < maze.maze_heigth; y++) {
			if (maze.maze_grid[maze.translateXY(x,y)] == '#' || maze.maze_grid[maze.translateXY(x,y)] == '1') {
				...
				*/
				...
				glEnable(GL_TEXTURE_2D);
				glBindTexture(GL_TEXTURE_2D,1);
				//glMaterialfv(GL_FRONT_AND_BACK,GL_AMBIENT_AND_DIFFUSE,vec_floor);
				glBegin(GL_QUADS);
					glNormal3f(0,0,1);
...
void initializeGhosts() {
    for (int i = 0 ; i < numGhosts; i++) {
		...
		ghost[i].set_cellWidth(cell_width);
